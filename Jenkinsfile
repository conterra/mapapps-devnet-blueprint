            ...
            steps {
                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: CREDENTIAL_ID, passwordVariable: 'USER_PW', usernameVariable: 'USER_NAME')]){
                        sh "mvn deploy -P brotli,compress,release -Dgulp.task=${jsBuild} -DdeployAtEnd=true -Dmaven.test.skip.exec=true -Dct-nexus.username=${USER_NAME} -Dct-nexus.password=${USER_PW}"
                    }
                }
            }
        }
